...
const Employees = () => {
  ...
  React.useEffect(() => {
    const getEmployees = async (index) => {
        const response = await axios.get("http://localhost:5000/api/employees?page=0", {withCredentials: true})
  ...
  const appendEmployees = async () => {
    const response = await axios.get(`http://localhost:5000/api/employees?page=${index}`, {withCredentials: true});
